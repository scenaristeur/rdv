<template>
    <div>
        ...
        <span v-if="map != null">
            <q-list separator bordered clickable :key="index"
                v-for="(layer, index) in map.getLayers().getArray().slice().reverse()">
                <q-item style="height: 10px;">
                    ...
                    <q-item-section avatar>
                        <q-icon name="keyboard_arrow_down" class="expansion_align cursor-pointer" size="25px"
                            @click="toggleSwitch(layer, $event)" v-if="!layer.get('isBaseMap')" />
                    </q-item-section>
                </q-item>
                <q-separator />
...
import layers from "../assets/layers/layers"
